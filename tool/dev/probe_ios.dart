...
Future<void> main(List<String> args) async {
  ...
  print('objectVersion: ${ios.objectVersion}  deploymentTarget: ${ios.deploymentTarget}');
  ...
  for (final c in t.buildConfigurations.values) {
    print('  ${c.name.padRight(22)} ${c.bundleIdentifier ?? "<unresolved>"}  display=${c.displayName ?? "-"}  team=${c.developmentTeam ?? "-"}');
  }
  print('schemes:');
  for (final s in ios.schemes.values) {
    print('  ${s.name.padRight(14)} shared=${s.shared} launch=${s.buildConfiguration} archive=${s.archiveConfiguration}');
  }
